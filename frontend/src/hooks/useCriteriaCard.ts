...
export const criteriaCardPool: CriteriaCard[] = [
  ...
  { id: 45, criteriaSlots: 8, irrelevantCriteria: [] },
  { id: 46, criteriaSlots: 8, irrelevantCriteria: [] },
  { id: 47, criteriaSlots: 8, irrelevantCriteria: [] },
...
export const useCriteriaCard = (verifier: Verifier, index: number) => {
  ...
  const cardImageSources = useMemo(
    () => getCardImageSources(card, language),
    [card, language]
  );
  ...
  return {
    card,
    cardImage: cardImageSources.primary,
    cardImageFallback: cardImageSources.fallback,
    toggleCriteria,
    letters,
    toggleLetter,
  };
